<?php
    ...
    if ($_POST['send'] ?? null) {
        ...
        while(true) {
                ...
                $response = $app->graphQL([
                    'query' => '
                        query MyQuery ($connectionId: Int!, $after: String) {
                          customContents(connectionIdExport: $connectionId, first: 2, after: $after) {
                            edges {
                              node {
                                contentId
                                status
            ...
            foreach ($response['data']['customContents']['edges'] as $edge) {
                $product = $edge['node'];
            ...
            if (!($response['data']['customContents']['pageInfo']['hasNextPage'] ?? false)) {
                break;
            }
            $after = $response['data']['customContents']['pageInfo']['endCursor'];
        }
